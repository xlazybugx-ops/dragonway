...
Промпты для генерации тест-данных и проверки консистентности стиля.

## Как пользоваться
Собирай промпт как `MASTER-preamble + блок этой категории + NEGATIVE`. Неизменяемый стиль — в
`prompts/AI_PROMPT_LIBRARY.md`; параметризованные блоки — в `prompts/PROMPT_TEMPLATES.md`; мастер-
промпты — `prompts/MASTER_*.md`. Имена выходных файлов — строго по `design/ASSET_NAMING.md`.

## Обязательный стиль (кратко)
...
## Приёмка
Ассет проходит чек-лист `design/STYLE_GUIDE.md`. Брак → перегенерация с той же преамбулой. Утверждённые
образцы класть рядом как референсы для image-to-image.
